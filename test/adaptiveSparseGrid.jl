...
for quad_rule in (clenshawcurtis01_nested, gausspatterson01_nested)
    ...
    @testset "AdaptiveSparseGrid" begin
        is_CC = quad_rule == clenshawcurtis01_nested
        @testset "One dimensional" begin
            ...
            # Check if we're using CC or gausspatterson
            needed_level = is_CC ? ceil(Int, log2(10)) : ceil(Int, log2((10+2)÷2))
            ...
            expected_evals = is_CC ? 2^(needed_level + 1) + 1 : 2^(needed_level + 1) - 1
            @test length(eval_dict) == expected_evals

            expect_pts, expect_wts = quad_rule(needed_level + Int(is_CC))
        ...
        @testset "Two dimensional" begin
            ...
            expected_maxDegrees = is_CC ? [5,4] : [3,3]
    ...
    @testset "High dimensional" begin
        ...
        asg = AdaptiveSparseGrid(mset_base, ntuple(_ -> quad_rule, dim), tol=5eps(); is_valid_midx)
